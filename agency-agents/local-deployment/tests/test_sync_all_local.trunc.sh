...
case_apply_success_and_protections() {
  ...
  read -r action signature allowed_signers < <(build_auth_bundle "$tmp" aicc-supervisor-authorization supervisor-approver "$home/action.json" "$MANIFEST_PATH" "" "" "$home")
  ...
  if ! HOME="$home" PROJECT="$home" "$SYNC_SCRIPT" --home "$home" --apply --manifest "$MANIFEST_PATH" --source-root "$SOURCE_ROOT" --project "$home" --action-file "$action" --signature-file "$signature" --allowed-signers "$allowed_signers" --ledger "$home/ledger.json" >"$report_file"; then
...
case_missing_qwen_kimi_targets() {
  ...
  local src_root="$tmp_root/source"
  cp -R "$SOURCE_ROOT/." "$src_root"

  rm -rf "$home/.qwen" "$home/.config/kimi"

  read -r action signature allowed_signers < <(build_auth_bundle "$tmp_root" aicc-supervisor-authorization supervisor-approver "$home/action.json" "$MANIFEST_PATH" "" "" "$home")

  report_file="$(mktemp)"
  if ! HOME="$home" PROJECT="$home" "$SYNC_SCRIPT" --home "$home" --apply --manifest "$MANIFEST_PATH" --source-root "$src_root" --project "$home" --action-file "$action" --signature-file "$signature" --allowed-signers "$allowed_signers" --ledger "$home/ledger.json" >"$report_file"; then
...
case_replay_rejected() {
  ...
  if ! HOME="$home" PROJECT="$home" "$SYNC_SCRIPT" --home "$home" --apply --manifest "$MANIFEST_PATH" --source-root "$SOURCE_ROOT" --project "$home" --action-file "$action" --signature-file "$signature" --allowed-signers "$allowed_signers" --ledger "$home/ledger.json" >"$report1"; then
    rm -rf "$home" "$tmp" "$report1" "$report2"
    return 1
  fi

  if HOME="$home" PROJECT="$home" "$SYNC_SCRIPT" --home "$home" --apply --manifest "$MANIFEST_PATH" --source-root "$SOURCE_ROOT" --project "$home" --action-file "$action" --signature-file "$signature" --allowed-signers "$allowed_signers" --ledger "$home/ledger.json" >"$report2"; then
...
case_bad_namespace_or_principal() {
  ...
  read -r action signature allowed_signers < <(build_auth_bundle "$tmp" "$ns" "$principal" "$home/action.json" "$MANIFEST_PATH" "" "" "$home")

  if HOME="$home" PROJECT="$home" "$SYNC_SCRIPT" --home "$home" --apply --manifest "$MANIFEST_PATH" --source-root "$SOURCE_ROOT" --project "$home" --action-file "$action" --signature-file "$signature" --allowed-signers "$allowed_signers" --ledger "$home/ledger.json" >"$report"; then
